...
def backup():
    ...
    response, status_code = backup_git(file_name, bucket_name, gitlab_user, gitlab_repo, gitlab_branch)
...
def wiki():
    ...
    response, status_code = call_wiki_lambda(title)
    send_whatsapp_messages(response)
    return response, status_code
...
def init_gitlab():
    ...
    response, status_code = send_users_to_lambda(sheet)
    send_whatsapp_messages(response)
    return response, status_code
...
def xlsx():
    ...
    response, status_code = csv_to_xlsx(csv_data=csv_data, commit_message=commit_message, output_file_name=output_file_name)
    send_whatsapp_messages(response)
    return response, status_code
